...
export const SocialLink = styled.a`
  ...
  background: ${({ theme }) =>
    theme.accentGradient || "linear-gradient(135deg, #6e8efb, #a777e3)"};
  box-shadow: ${({ theme }) =>
  ...
  /* hover effects */
  &:hover {
    transform: scale(1.35) translateY(-8px) rotate(-5deg);
    animation: ${float} 1s ease-in-out infinite;
    background: ${({ theme }) =>
      theme.hoverGradient || "linear-gradient(135deg, #ff758c, #ff7eb3)"};
...
export const Sub = styled.p`
  ...
  &:hover {
    ...
    cursor: ${(p) => (p.hoverCursor ? p.hoverCursor : "default")};
  }
  ...
  ${(p) =>
    p.highlight &&
    css`
      background: linear-gradient(90deg, #ff758c, #ff7eb3, #6e8efb);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      font-weight: 700;
  ...
  /* optional underline sparkle effect */
  ${(p) =>
    p.sparkle &&
    css`
      position: relative;
  ...
  @media (max-width: 768px) {
    line-height: ${(p) => p.lineHeightMobile || 1.6};
    font-size: clamp(${(p) => p.minFontMobile || 14}px, ${(p) => p.fontVW || 3}vw, ${(p) => p.maxFontMobile || 16}px);
  }
`;
export const DownloadCV = styled.a`
  ...
  background: linear-gradient(135deg, #6e8efb, #ff7eb3);
  color: #fff;
...
export const ProfileImage = styled.img`
  width: 100%;
  border-radius: 20px;
  border: 1px solid ${(props) => props.border || props.theme.border};
  box-shadow: ${(props) => props.shadow || props.theme.glow};
  transition: transform 0.4s ease, box-shadow 0.4s ease;

  &:hover {
    transform: scale(1.03) translateY(-4px);
    box-shadow: 0 12px 36px rgba(0,0,0,0.35), 0 0 24px rgba(255,255,255,0.1) inset;
  }
...
// Card title with ultra-complex styles
export const CardTitle = styled.h3`
  ...
  /* Pseudo-elements for layered underline effect */
  &::before, &::after {
    content: "";
    position: absolute;
    left: 0;
