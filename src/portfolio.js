...
import SkillsSection from "./skills"; // now uses styled lists
    ...
    <ThemeProvider theme={theme}>
      <Page>
        <TopNav>
          <TopBar>
            <NavLinks>
              {["home","objective","education","training","projects","skills","languages"].map(id => 
                <a key={id} onClick={() => document.getElementById(id)?.scrollIntoView({ behavior: "smooth" })}>{id}</a>
              )}
            </NavLinks>
            <SocialContainer>
        ...
        <Shell>
          ...
          <TrainingSection />
          <ProjectsSection />
          <SkillsSection />  {/* now styled list system */}
          <LanguagesSection />
        </Shell>
        ...
        {modal && (
          <motion.div
            ...
            style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "grid", placeItems: "center", zIndex: 60, padding: 24 }}
          >
            <motion.img
              src={modal}
              alt="Preview"
        ...
        <Fab onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} title="Back to top">
          <FaChevronUp />
        </Fab>
      </Page>
    </ThemeProvider>
